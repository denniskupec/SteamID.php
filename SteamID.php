<?php
/**
 * The SteamID class provides an easy way to work with SteamIDs and makes
 * conversions easy. Ported from SteamKit.
 *
 * This library requires the GMP module to be installed.
 *
 * Forked From: {@link https://github.com/xPaw/SteamID.php}
 *
 * Added PHP7 type hinting and extra static methods for ease of use.
 *
 * @author Dennis Kupec
 * @license MIT
 */
...
class SteamID
{
   ...
   /* Special flags for Chat accounts */
   ...
   public function __construct($Value=null)
   {
      ...
      if(preg_match('/^\\[([AGMPCgcLTIUai]):([0-4]):([0-9]{1,10})(:([0-9]+))?\\]$/', $Value, $Matches) === 1) {
   ...
   public function RenderSteam3(): string
   {
      ...
      switch($AccountType)
      {
         case self::TypeChat:
         {
            ...
            if($AccountInstance & self::InstanceFlagLobby) {
               $AccountTypeChar = 'L';
            }
   ...
   public function IsValid(): bool
   {
      ...
      if($AccountType <= self::TypeInvalid || $AccountType >= 11) {
         return false;
      }
   ...
   public static function RenderSteam2($input): string
   {
      $steamid = new self($input);

      return $steamid->RenderSteam2();
   }

   public static function RenderSteam64($input): string
   {
      $steamid = new self($input);
   ...
   public static function RenderSteam2($input)
   {
